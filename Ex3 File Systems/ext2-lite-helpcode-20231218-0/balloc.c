                                     ...
                                     ext2_fsblk_t start_blk, unsigned int count)
{
	...
	if (in_range(le32_to_cpu(desc->bg_block_bitmap), start_blk, count))
		return 0;

	if (in_range(le32_to_cpu(desc->bg_inode_bitmap), start_blk, count))
		return 0;

	if (in_range(start_blk, le32_to_cpu(desc->bg_inode_table), sbi->s_itb_per_group))
		return 0;

	if (in_range(end_blk, le32_to_cpu(desc->bg_inode_table), sbi->s_itb_per_group))
		return 0;
